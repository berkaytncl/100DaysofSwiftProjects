...
extension GameScene {
    ...
    private func createEnemy(forceBomb: ForceBomb = .random) {
        ...
        let randomAngularVelocity = CGFloat(Int.random(in: -3...3))
    ...
    private func endGame(triggeredByBomb: Bool = false) {
        guard isGameEnded == false else { return }
        ...
        if triggeredByBomb {
            for livesImage in livesImages {
                ...
                livesImage.xScale = 1.3
                livesImage.yScale = 1.3
                livesImage.run(SKAction.scale(by: 1, duration: 0.1))
            }
        }
    }
}
